...
export default function ServicesClient({ categories, whatsappNumber }: { categories: any, whatsappNumber: string }) {
    const { t } = useLanguage();
        ...
        <div className="min-h-screen py-24 relative overflow-hidden">
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="text-center mb-16 max-w-3xl mx-auto">
                    <h1 className="font-serif text-4xl sm:text-6xl font-bold text-white mb-6">{t("services.title")}</h1>
                ...
                {categories.length === 0 ? (
                    ...
                    <div className="space-y-20">
                        ...
                        <div className="mt-20 max-w-3xl mx-auto bg-primary/10 border border-gold/30 rounded-2xl p-8 backdrop-blur-sm shadow-xl">
                            <h3 className="text-2xl font-serif text-gold mb-6 text-center border-b border-gold/20 pb-4">{t("services.bookInfoTitle")}</h3>
                            <div className="space-y-4 text-lavender-dark text-lg font-light">
                                <p className="flex items-start">
                                    <span className="text-gold mr-3 mt-1">👉</span>
                                    <span>{t("services.bookInfo1")}</span>
                                </p>
                                <p className="flex items-start">
                                    <span className="text-gold mr-3 mt-1">👉</span>
                                    <span>{t("services.bookInfo2")}</span>
                                </p>
                                <div className="flex items-start mt-8 pt-6 border-t border-primary/30">
                                    <span className="text-gold mr-4 text-2xl mt-1">📲</span>
                                    <div>
                                        <strong className="text-white block mb-1 font-medium">{t("services.payMode")}</strong>
                                        <span className="text-gold-light font-semibold tracking-wide">{t("services.payModeDetail")}</span>
                                        <p className="text-sm mt-2 opacity-80 italic">{t("services.payNote")}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
